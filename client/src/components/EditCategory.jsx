...
const EditCategory = ({ close, fetchData, data: CategoryData }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!data._id || !data.name || !data.image) {
  ...
  const handleUploadCategoryImage = async (e) => {
    ...
    try {
      ...
      setData(prev => ({ ...prev, image: imageRes.data.url }));
    } catch (err) {
      ...
      <div className='bg-white max-w-4xl w-full p-4 rounded'>
        ...
        <form className='my-3 grid gap-3' onSubmit={handleSubmit}>
          ...
          <button
            ...
            className={`
              ${data.name && data.image && !loading ? "bg-primary-200 hover:bg-primary-100" : "bg-gray-300 cursor-not-allowed"} 
