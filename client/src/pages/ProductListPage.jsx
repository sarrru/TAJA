...
const ProductListPage = () => {
  ...
  return (
    <section className="font-sans bg-gray-50 min-h-screen py-4 px-2">
      <div className="container mx-auto grid grid-cols-[90px,1fr] md:grid-cols-[200px,1fr] lg:grid-cols-[280px,1fr] gap-4">
        {/* Sidebar: Subcategories */}
        <div className="min-h-[88vh] max-h-[88vh] overflow-y-auto scrollbarCustom bg-white shadow rounded p-2">
          {
            DisplaySubCatory.map((s, index) => {
              const link = `/${valideURLConvert(s?.category[0]?.name)}-${s?.category[0]?._id}/${valideURLConvert(s.name)}-${s._id}`
              return (
                <Link
                  key={s._id}
                  to={link}
                  className={`w-full flex items-center gap-4 p-2 rounded mb-1 transition-all duration-150 hover:bg-green-100 ${
