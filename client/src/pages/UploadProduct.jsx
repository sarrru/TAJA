    ...
    <section>
      ...
      <div className='grid p-3'>
        <form className='grid gap-4' onSubmit={handleSubmit}>
              ...
              {data.image.map((img, index) => (
                <div key={img + index} className='h-20 mt-1 w-20 min-w-20 bg-blue-50 border relative group'>
                  <img
                    ...
                    onClick={() => setViewImageURL(img)}
                  />
                  <div onClick={() => handleDeleteImage(index)} className='absolute bottom-0 right-0 p-1 bg-red-600 text-white hidden group-hover:block cursor-pointer'>
                    <MdDelete />
                  </div>
                </div>
              ))}
          ...
          {["unit", "stock", "price", "discount"].map((field, index) => (
            <div key={index} className='grid gap-1'>
              <label htmlFor={field} className='font-medium'>
                {field === "unit" ? "Unit" : field === "stock" ? "Number of Stock" : field.charAt(0).toUpperCase() + field.slice(1)}
              </label>
              <input
                id={field}
          ...
          {Object.keys(data.more_details).map((k, index) => (
            <div key={k + index + "_extra"} className='grid gap-1'>
              <label htmlFor={k} className='font-medium'>{k}</label>
              <input
                id={k}
                type='text'
